...
export default function NudgeBadge() {
  ...
  const routeLabel = s.routePrimary === 'telegram-stub' ? 'Telegram (stub)' : s.routePrimary;
  const plannerLocal = new Date(s.weeklyPlannerLocal).toLocaleString('ru-RU', {
  ...
  return (
    <div className="text-xs px-2 py-1 rounded bg-zinc-800 text-zinc-200 border border-zinc-700">
      ...
      <span>route <span className="font-semibold">{routeLabel}</span></span>{' '}
      <span>• dedupe {s.dedupeWindowSec}s</span>{' '}
      <span>• planner {plannerLocal}</span>
    </div>
  );
}
